...
def load_data():
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        for group in file_groups:
            filename = DATA_DIR / group["pattern"].format(year)
            if filename.exists():
                # Robust Loader Logic
                loaded = False
                            ...
                            if len(df.columns) > 1:
                                ...
                                if mask_no_id.any():
                                    ...
                                    if "tourney_name" not in df.columns:
                                        df["tourney_name"] = "Unknown"
                                    df.loc[mask_no_id, "tourney_name"] = df.loc[mask_no_id, "tourney_name"].fillna("Unknown")
                                    df.loc[mask_no_id, "tourney_id"] = (
                                        "eu_" + df.loc[mask_no_id, "tourney_name"].astype(str).str.replace(" ", "_").str.lower()
                                    )
...
def load_players():
    ...
    if players_path.exists():
        ...
        players = pd.read_csv(players_path, usecols=["player_id", "hand"])
...
def restructure_data(df, players_df):
    ...
    # P1 = Winner
    w_to_p1 = {
        col: col.replace("winner_", "p1_").replace("w_", "p1_")
        for col in df.columns
        if "winner_" in col or col.startswith("w_")
    }
    l_to_p2 = {
        col: col.replace("loser_", "p2_").replace("l_", "p2_")
        for col in df.columns
        if "loser_" in col or col.startswith("l_")
    }
    ...
    # P1 = Loser
    l_to_p1 = {
        col: col.replace("loser_", "p1_").replace("l_", "p1_")
        for col in df.columns
        if "loser_" in col or col.startswith("l_")
    }
    w_to_p2 = {
        col: col.replace("winner_", "p2_").replace("w_", "p2_")
        for col in df.columns
        if "winner_" in col or col.startswith("w_")
    }
