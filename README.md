...
- `uncertainty_score` pre-match rule-based con pesi configurabili (`MP_UNCERTAINTY_WEIGHTS`) e gate `recommendation_allowed` via `MP_MAX_UNCERTAINTY_FOR_RECO`.
- Calibrazione separata per livello (`A/C/F`) con fallback automatico se campione sotto `MP_N_MIN_CALIB` e tracciamento nel validation report.
- Report affidabilita per bucket su probabilita, uncertainty e odds in `runs/reliability_by_p_bucket.csv`, `runs/reliability_by_uncertainty_bucket.csv`, `runs/reliability_by_odds_bucket.csv`.
- Smoke test `python scripts/smoke_uncertainty_and_calib.py` per verifica rapida pipeline uncertainty + calibrazione per livello.
- Snapshot baseline riproducibile in `golden/<run_id>/` con `manifest.json` (SHA256/size per file) via `python scripts/snapshot_golden_run.py`.
- Regression gate automatico (`python scripts/regression_gate.py`) con hard-fail su OOS/coerenza/drawdown/fold/ROI walk-forward e output `runs/<run_id>/regression_gate_result.json`.
...
Override baseline strategy via env (senza modificare codice):
- `BT_BASELINE_MIN_EDGE`
- `BT_BASELINE_MIN_EV`
- `BT_BASELINE_MIN_CONFIDENCE`
- `BT_BASELINE_PROB_SHRINK`
- `BT_BASELINE_MIN_ODDS`
- `BT_BASELINE_MAX_ODDS`
- `BT_BASELINE_MIN_SIGNAL_SCORE`
...
- `runs/backtest_walkforward_bet_records.csv` (diagnostica bet per fold/policy)
- `runs/backtest_walkforward_policy_audit.csv` (audit policy per fold/policy)
- `runs/<run_id>/regression_gate_result.json` (esito gate hard-fail/warn vs golden)
- `runs/<run_id>/analysis_walkforward.md` (analisi fold-level Q3/Q4 o fold peggiori)
...
Configurazione GO validata (2026-02-10):
```bash
MP_POLICY_ENABLED=false \
MP_ENABLE_BUCKET_REPORTS=true \
BT_RESTRICTED_TUNING_MODE=true \
BT_FOLD_FREQ=quarter \
BT_BASELINE_MIN_ODDS=1.7 \
BT_BASELINE_MIN_CONFIDENCE=0.71 \
python backtest.py
```
...
Snapshot golden della run corrente (latest in `runs/`):
```bash
python scripts/snapshot_golden_run.py
```

Snapshot golden di una run specifica:
```bash
python scripts/snapshot_golden_run.py --run-id bt_YYYYMMDD_HHMMSS_xxxxxxxx
```

Output:
- `golden/<run_id>/...` con report e artifact copiati
- `golden/<run_id>/manifest.json` con checksum SHA256 + size file + git hash (se disponibile)
- `golden/latest` aggiornato automaticamente

## Regression Gate

Confronto run corrente vs golden (hard-fail + warning):
```bash
python scripts/regression_gate.py --golden golden/latest
```

Confronto run esplicita:
```bash
python scripts/regression_gate.py --run-id bt_YYYYMMDD_HHMMSS_xxxxxxxx --golden golden/latest
```

Hard fail (exit code != 0) se presenti i campi:
- `oos_gate.pass != true`
- `report_coherence.pass != true`
- `max_drawdown_pct > 5`
- `n_valid_folds < 3`
- `walkforward_overall_baseline_roi < 0`

Warnings (exit code 0):
- regressioni su calibrazione/reliability oltre tolleranze configurabili (`--tol-*`).

Output:
- tabella testuale in stdout
- `runs/<run_id>/regression_gate_result.json`

## Validation Suite (One Command)

Comando completo:
```bash
python scripts/validate_all.py --golden golden/latest
```

Modalita solo gate (no smoke, no backtest):
```bash
python scripts/validate_all.py --no-backtest --skip-smokes --run-id bt_YYYYMMDD_HHMMSS_xxxxxxxx --golden golden/latest
```
...
Nota: la suite materializza la run in `runs/<run_id>/` e poi esegue `regression_gate`.

## Analisi Diagnostica Fold

Genera report markdown fold-level:
```bash
python scripts/analyze_walkforward_folds.py --run-id bt_YYYYMMDD_HHMMSS_xxxxxxxx
```

Output:
- `runs/<run_id>/analysis_walkforward.md`
...
Smoke test comparativo policy ON/OFF (esegue due backtest e salva confronto):
```bash
python scripts/smoke_policy_toggle.py
```
Output:
- `runs/policy_toggle_comparison.json`
Note:
- usa default runtime veloci (`BT_RESTRICTED_TUNING_MODE=true`, `BT_FOLD_FREQ=halfyear`) se non gia impostati nell'ambiente.

Confronto policy OFF vs policy ON soft:
```bash
python scripts/compare_policy_variants.py
```
Preset SOFT usato nello script:
...
Modalita diff-only tra due run esistenti:
```bash
python scripts/compare_policy_variants.py --skip-runs --run-id-off bt_OFF --run-id-soft bt_SOFT
```
