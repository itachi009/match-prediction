...
WALKFORWARD_BET_RECORDS_FILE = RUNS_DIR / "backtest_walkforward_bet_records.csv"
WALKFORWARD_POLICY_AUDIT_FILE = RUNS_DIR / "backtest_walkforward_policy_audit.csv"
...
def apply_baseline_env_overrides(config):
    cfg = dict(config)
    overrides = {
        "BT_BASELINE_MIN_EDGE": ("min_edge", _as_float),
        "BT_BASELINE_MIN_EV": ("min_ev", _as_float),
        "BT_BASELINE_MIN_CONFIDENCE": ("min_confidence", _as_float),
        "BT_BASELINE_PROB_SHRINK": ("prob_shrink", _as_float),
        "BT_BASELINE_MIN_ODDS": ("min_odds", _as_float),
        "BT_BASELINE_MAX_ODDS": ("max_odds", _as_float),
        "BT_BASELINE_MIN_SIGNAL_SCORE": ("min_signal_score", _as_float),
    }
    applied = {}
    for env_key, (cfg_key, caster) in overrides.items():
        raw = os.getenv(env_key)
        if raw is None:
            continue
        cfg[cfg_key] = caster(raw, cfg.get(cfg_key))
        applied[cfg_key] = cfg[cfg_key]

    cfg["min_confidence"] = clip01(cfg.get("min_confidence", 0.64))
    cfg["prob_shrink"] = clip01(cfg.get("prob_shrink", 0.62))
    cfg["min_odds"] = max(1.01, _as_float(cfg.get("min_odds"), 1.55))
    cfg["max_odds"] = max(cfg["min_odds"], _as_float(cfg.get("max_odds"), 3.00))
    cfg["min_edge"] = _as_float(cfg.get("min_edge"), 0.06)
    cfg["min_ev"] = _as_float(cfg.get("min_ev"), 0.06)
    cfg["min_signal_score"] = max(0.0, _as_float(cfg.get("min_signal_score"), 0.005))

    if applied:
        print(f"[BASELINE] Env overrides applied: {applied}")
    return cfg


BASELINE_CONFIG = apply_baseline_env_overrides(BASELINE_CONFIG)
...
def run_walk_forward_validation(df_sim, baseline_config, run_context=None):
    ...
    for i in range(1, len(periods)):
        ...
        for rec in (base_test.get("bet_records") or []):
            row = dict(rec)
            row["test_period"] = str(test_period)
            row["policy_variant"] = "baseline"
            row["use_tuned_policy"] = bool(use_tuned_policy)
            wf_bet_records.append(row)
        for rec in (tuned_test.get("bet_records") or []):
            row = dict(rec)
            row["test_period"] = str(test_period)
            row["policy_variant"] = "tuned"
            row["use_tuned_policy"] = bool(use_tuned_policy)
            wf_bet_records.append(row)
        for rec in (base_test.get("policy_audit") or []):
            row = dict(rec)
            row["test_period"] = str(test_period)
            row["policy_variant"] = "baseline"
            row["use_tuned_policy"] = bool(use_tuned_policy)
            wf_policy_audit.append(row)
        for rec in (tuned_test.get("policy_audit") or []):
            row = dict(rec)
            row["test_period"] = str(test_period)
            row["policy_variant"] = "tuned"
            row["use_tuned_policy"] = bool(use_tuned_policy)
    ...
    diagnostics_files = {
        "walkforward_bet_records": None,
        "walkforward_bet_records_rows": 0,
        "walkforward_policy_audit": None,
        "walkforward_policy_audit_rows": 0,
    }
    wf_bet_df = pd.DataFrame(wf_bet_records)
    if not wf_bet_df.empty:
        wf_bet_df.to_csv(WALKFORWARD_BET_RECORDS_FILE, index=False)
        diagnostics_files["walkforward_bet_records"] = str(WALKFORWARD_BET_RECORDS_FILE)
        diagnostics_files["walkforward_bet_records_rows"] = int(len(wf_bet_df))
        print(f"    WF bet records salvati in: {WALKFORWARD_BET_RECORDS_FILE}")
    wf_policy_df = pd.DataFrame(wf_policy_audit)
    if not wf_policy_df.empty:
        wf_policy_df.to_csv(WALKFORWARD_POLICY_AUDIT_FILE, index=False)
        diagnostics_files["walkforward_policy_audit"] = str(WALKFORWARD_POLICY_AUDIT_FILE)
        diagnostics_files["walkforward_policy_audit_rows"] = int(len(wf_policy_df))
        print(f"    WF policy audit salvato in: {WALKFORWARD_POLICY_AUDIT_FILE}")
